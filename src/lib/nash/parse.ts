// Nash parser — fork of the UPLC parser with `let` support.
...
class Parser {
  ...
  private parseParenTerm(): NashTerm<Name> {
  ...
  private parseLet(): NashTerm<Name> {
    this.expect("let");
    ...
    if (bindings.length === 0) {
      throw new ParseError(
        `let requires at least one binding at position ${this.current.position}`,
      );
    }
  ...
  private parseLambda(): NashTerm<Name> {
    this.expect("lam");
  ...
  private parseDelay(): NashTerm<Name> {
    this.expect("delay");
    const term = this.parseTerm();
    this.expect("rparen");
  ...
  private parseForce(): NashTerm<Name> {
    this.expect("force");
    const term = this.parseTerm();
    this.expect("rparen");
  ...
  private parseBuiltin(): NashTerm<Name> {
    this.expect("builtin");
  ...
  private parseCase(): NashTerm<Name> {
    if (this.isBeforeV1_1_0()) {
  ...
  private parseApply(): NashTerm<Name> {
    this.expect("lbracket");

    const terms: NashTerm<Name>[] = [];
    while (!this.is("rbracket")) {
    ...
    let result: NashTerm<Name> = terms[0]!;
  ...
  private parseConstantTerm(): NashTerm<Name> {
    this.expect("con");
...
export function parseNash(source: string): NashProgram<Name> {
